...
ICM_20948_Status_e  ICM_20948::clearInterrupts         ( void )
{
    ...
    status = ICM_20948_set_bank( &_device, 0 );                                                                                   if ( status != ICM_20948_Stat_Ok ) {
        return status;
    }
    status = ICM_20948_execute_r( &_device, AGB0_REG_INT_STATUS, (uint8_t *)&int_stat, sizeof(ICM_20948_INT_STATUS_t) );           if ( status != ICM_20948_Stat_Ok ) {
        return status;
    }
    status = ICM_20948_execute_r( &_device, AGB0_REG_INT_STATUS_1, (uint8_t *)&int_stat_1, sizeof(ICM_20948_INT_STATUS_1_t) );     if ( status != ICM_20948_Stat_Ok ) {
        return status;
    }
...
ICM_20948_Status_e  ICM_20948::startupDefault          ( void )
{
    ...
    if ( retval != ICM_20948_Stat_Ok ) {
        status = retval;    // sensors:    ICM_20948_Internal_Acc, ICM_20948_Internal_Gyr, ICM_20948_Internal_Mst
        return status;
    }
...
ICM_20948_Status_e  ICM_20948_I2C::stopMagnetometer    ( void )
{
    // If using the magnetometer through passthrough:
    i2cMasterPassthrough( true ); // Set passthrough mode to try to access the magnetometer (by default I2C master is disabled but you still have to enable the passthrough)
    ...
    // Try to set up magnetometer
    AK09916_CNTL2_Reg_t reg;
    reg.MODE = AK09916_mode_power_down;
